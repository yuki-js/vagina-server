...
public interface OAuthLoginAttemptMapper {
  ...
  @Insert(
      "INSERT INTO oauth_login_attempts (state_hash, auth_method, client_type, redirect_uri, code_challenge, "
          + "code_challenge_method, expires_at, consumed_at, sysmeta, created_at, updated_at) VALUES "
          + "(#{stateHash}, #{authMethod, typeHandler=app.vagina.server.mapper.type.AuthMethodTypeHandler}, "
          + "#{clientType, typeHandler=app.vagina.server.mapper.type.ClientTypeHandler}, #{redirectUri}, "
          + "#{codeChallenge}, #{codeChallengeMethod}, #{expiresAt}, #{consumedAt}, #{sysmeta}::jsonb, #{createdAt}, #{updatedAt})")
  @Options(useGeneratedKeys = true, keyProperty = "id")
  void insert(OAuthLoginAttempt oauthLoginAttempt);

  @Select(
      "SELECT id, state_hash, auth_method, client_type, redirect_uri, code_challenge, code_challenge_method, "
  ...
  @Update(
      "UPDATE oauth_login_attempts SET auth_method = "
          ...
          + "client_type = #{clientType, typeHandler=app.vagina.server.mapper.type.ClientTypeHandler}, "
          + "redirect_uri = #{redirectUri}, code_challenge = #{codeChallenge}, "
          + "code_challenge_method = #{codeChallengeMethod}, expires_at = #{expiresAt}, consumed_at = #{consumedAt}, "
